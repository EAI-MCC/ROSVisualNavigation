...
from PPO.networks_nav_crowd import PolicyNet, VNet

class Agent():
    def __init__(self, obsv_dim, map_size, act_dim, lr, device="cpu", discount_factor=0.99,
                 target_kl=0.01, ent_coef=0.01, rho=0.2, clip_v=True, is_train=True):
    ...
    def compute_loss(self, inputs, train_pi_iters, train_v_iters):
        obsv, imap, hmap, acts, logp_a_old, advs, rets, vals, goal = inputs

        for i in range(train_pi_iters):
            prob = self.pi(obsv, imap, hmap, goal)
            logp = (prob + self.small_number).log()
            logp_a = logp.gather(dim=-1, index=acts.unsqueeze(-1)).squeeze(-1)
            ratio = (logp_a - logp_a_old).exp()

            loss_pi = -torch.min(ratio*advs, ratio.clamp(1-self.rho, 1+self.rho)*advs).mean()
        ...
        info = {"LossPi": loss_pi_old, "LossV": loss_v_old, "KL": approx_kl, "Entropy": ent_old,
                "ClipFrac": clip_frac, "DeltaLossPi": loss_pi.item() - loss_pi_old,
                "DeltaLossV": loss_v.item() - loss_v_old, "V": v.mean().item()}
        return info 
    
    def compute_value(self, state, imap, hamp, goal):
        with torch.no_grad():
            v = self.v(state, imap, hmap, goal)
        return v
    ...
    def get_param(self):
        state_dicts = {'pi': self.pi.state_dict(), 'v': self.v.state_dict(),
